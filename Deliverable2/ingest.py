...
# FIX #4: was "BAAI/bge-small-en" (no version suffix) — retriever.py uses "bge-small-en-v1.5".
# Mismatched checkpoints silently break dense search because stored and query vectors
# come from different models. Both files now pin to v1.5.
...
def upsert_qdrant(
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        points = [
            PointStruct(
                # FIX #9: Qdrant requires uint or UUID point IDs — not raw hex strings.
                # Interpret the 16-char hex chunk_id as a 64-bit unsigned integer.
                id=int(c.chunk_id, 16),
                vector=v,
                payload={
                    "chunk_id":      c.chunk_id,   # keep original string in payload for lookup
                    "doc_id":        c.doc_id,
                    "title":         c.title,
                    "authors":       c.authors,
                    "year":          c.year,
