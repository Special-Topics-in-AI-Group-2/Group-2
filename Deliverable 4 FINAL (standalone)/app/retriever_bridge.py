...
class D1RetrieverAdapter:
    ...
    @staticmethod
    def _load_d1_retriever_cls():
        """Import D1's HybridKNNRetriever WITHOUT clashing with D2's retriever.py.

        Both ``src/retriever.py`` (D1) and ``app/retriever.py`` (D2) exist and a
        bare ``import retriever`` resolves to whichever is on sys.path first —
        a real ambiguity.  We load the D1 module from its explicit file path
        under a unique module name so the two never collide.
        """
        import importlib.util
        import os

        d1_path = os.path.join(os.path.dirname(__file__), "..", "src", "retriever.py")
        d1_path = os.path.abspath(d1_path)
        # Ensure ``import src...`` inside the D1 module resolves.
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
        import sys
        if project_root not in sys.path:
            sys.path.insert(0, project_root)
        spec = importlib.util.spec_from_file_location("d1_retriever_mod", d1_path)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)  # type: ignore[union-attr]
        return module.HybridKNNRetriever

    def __init__(self, config: Any, chunks: list) -> None:
        D1Retriever = self._load_d1_retriever_cls()
