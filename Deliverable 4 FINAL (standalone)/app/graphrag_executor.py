...
class GraphRAGExecutor:
    ...
    def __init__(
        ...
        slm_backend: str | None = None,        # D4: None/"extractive" -> deterministic renderer
        slm_base_model: str | None = None,     # D4: override SLM base model
    ) -> None:
        ...
        self.slm_base_model = slm_base_model
        ...
        self._generator = None          # D4: lazy SLM answer generator
    ...
    def _get_generator(self):
        """Lazily build the D4 SLM answer generator (extractive/base/tuned).

        Only ``base`` / ``tuned`` load a model; anything else (None,
        ``extractive``, or an unrecognised value such as the Swagger default
        ``"string"``) uses the deterministic grounded renderer — so a stray
        backend value never triggers a multi-GB model download.
        """
        if self.slm_backend not in ("base", "tuned"):
            return None
        if self._generator is None:
            try:
                from slm import AnswerGenerator, SLMConfig
                kw = {"backend": self.slm_backend}
                if self.slm_base_model:
                    kw["base_model"] = self.slm_base_model
                self._generator = AnswerGenerator(SLMConfig(**kw), verbose=self.verbose)
            except Exception as exc:  # noqa: BLE001
                self._log(f"[GraphRAGExecutor] SLM generator unavailable: {exc}")
    ...
    def answer(
        ...
        # D4: when an SLM backend is configured, the prose is generated by the
        # (optionally PEFT/QLoRA-tuned) model, but the Sources block and the
        # structured citation list are still derived from the retrieved chunks —
        # so provenance stays grounded even though the wording is generated.
        generator = self._get_generator()
        if generator is not None:
            gen_out = generator.generate(query, blended)
            _, refs = build_answer_with_citations(query, blended)
            sources_block = "\n".join(
                f"  [{r['marker']}] {r['title']}, {r['page_range']}"
                + (f" — {r['source_pdf']}" if r.get("source_pdf") else "")
                for r in refs
            )
            answer_text = f"{gen_out['answer']}\n\nSources:\n{sources_block}"
            used_mode = f"{used_mode}+slm:{gen_out['backend']}"
        else:
...
def main() -> None:
    ...
    parser.add_argument(
        "--slm-backend", choices=["extractive", "base", "tuned"], default="extractive",
        help="D4 answer generator: extractive (default), base (zero-shot SLM), or tuned (PEFT/QLoRA).",
    )
    parser.add_argument(
        "--slm-base-model", default=None,
        help="Override the SLM base model (e.g. sshleifer/tiny-gpt2 for a CPU smoke test).",
    )
    ...
    with GraphRAGExecutor(
        ...
        slm_base_model=args.slm_base_model,
    ) as executor:
