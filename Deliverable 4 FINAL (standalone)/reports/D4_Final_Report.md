...
> Per-member, unedited AI chat-history links are in §11 and `reports/AI_CHAT_LOGS.md`.
...
`(backend, model, query, contexts)` turns repeated queries into ~0 ms hits.
...
|---|---|---|
| Abdulla Alshaiba | 21003190 | D1 streaming learner + AutoML |
| Essa Alshamsi | 22001369 | D2 ingestion + hybrid retrieval + API |
| Ghaith Alneaimi | 22001613 | D2 Neo4j graph build + Cypher |
| Salem Hafez | 22001171 | D3 GraphRAG executor + safety + ablation |
| Yousef Al Refaie | 22000613 | D4 SLM tuning + integration + report |

Per-member, unedited AI chat-history links (D1–D3) are collected in
`reports/AI_CHAT_LOGS.md`; each member adds their own D4 link there before
submission. (Ownership above is the primary lead; all members contributed across
deliverables — see the GitHub commit history.)
