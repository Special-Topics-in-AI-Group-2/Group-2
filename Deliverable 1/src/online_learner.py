...
class OnlineTopicClassifier:
    ...
    def learn(self, text: str, true_topic: str) -> bool:
        ...
        # ── 3. Update model ───────────────────────────────────────────────────
        # BagOfWords.learn_one updates the vocabulary and returns self, so
        # chaining .transform_one gives features that include new tokens from
        # this example.  Calling transform_one first would silently drop them.
        features = self._bow.learn_one(text).transform_one(text)
